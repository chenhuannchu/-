...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 6) {
//        UIViewController *viewController = [[CTMediator sharedInstance] CTMediator_viewControllerForDetail1];
//        // 获得view controller之后，在这种场景下，到底push还是present，其实是要由使用者决定的，mediator只要给出view controller的实例就好了
//        [self.navigationController pushViewController:viewController animated:YES];
        
        [[CTMediator sharedInstance] CTMediator_startContinuousLocation];
...
//        });
        ...
        NSTimer *timer = [NSTimer timerWithTimeInterval:5.0f repeats:YES block:^(NSTimer * _Nonnull timer) {
            
            NSDictionary *dic = [CTMediator sharedInstance].locationMessage;
            NSLog(@"dic = %@",dic);
        }];
        [[NSRunLoop currentRunLoop] addTimer:timer forMode:NSRunLoopCommonModes];
        [[NSRunLoop currentRunLoop] run];
        ...
        [[CTMediator sharedInstance] CTMediator_stopContinuousLocation];
    }
}
...
{
    if (_dataSource == nil) {
        _dataSource = @[@"present detail view controller", @"push detail view controller", @"present image", @"present image when error", @"show alert", @"table view cell",@"StartContinuousLocation",@"StopContinuousLocation"];
    }
    return _dataSource;
}
@end
